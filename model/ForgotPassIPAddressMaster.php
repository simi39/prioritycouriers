<?php
/**
 * This is ForgotPassIPAddressMaster file.
 * This file contains all the business logic code related to  forgotpass_ipaddress_attempts table.
 * 
 * TABLE_NAME:     forgotpass_ipaddress_attempts
 * PRIMARY_KEY :  userid
 * TABLE_COLUMNS: 
 * 
...
/**
 * @see ForgotPassIPData.php for Data class
 */
require_once(DIR_WS_MODEL . "ForgotPassIPAddressData.php");
...
class ForgotPassIPAddressMaster extends RMasterModel
{
	/**
	...
	public function create()
	{
		...
		return  $ForgotPassIPAddressMasterObj;
	}//create Function End
	 ...
	 */
	public function addForgotPassIPAddress($ForgotPassIPAddressData, $ThrowError=true,$QueryString=null)
	{
		if(empty($QueryString))
		{
			//build up insert query
			$FinalData = $ForgotPassIPAddressData->InternalSync(RDataModel::INSERT,"ip","attempts","lastlogin");
			$Query = "INSERT INTO forgotpass_ipaddress_attempts
 $FinalData";
		}
		else
		{
		...
		//echo $Query;
 		...
 		try {
			...
			if ($Rs->AffectedRows() ==  0) {
				throw new Exception(' Fail To Add New forgotpass_ipaddress_attempts
 Record');
			}
		} catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Add forgotpass_ipaddress_attempts
 Record ');
			} else {
				return false;
			}//if-else end
	 ...
	 */
	public function editForgotPassIPAddress($ForgotPassIPAddressData=null, $QueryString=null, $ThrowError=true)
	{
		...
		if($QueryString)
		{
			 ...
			 $UpdateData = $ForgotPassIPAddressData->InternalSync(RDataModel::UPDATE, "attempts","lastlogin");
			 $Query = 'UPDATE forgotpass_ipaddress_attempts
 SET '.$UpdateData.' WHERE ip = "'.$ForgotPassIPAddressData->ip.'"';
		}
		//echo $Query;
		//exit();
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit forgotpass_ipaddress_attempts Record ');
				return false;
	  		}
		} catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit forgotpass_ipaddress_attempts Record ');
			 } else {
				return false;
			 }
    	}// try-catch end
	 ...
	 *
	 */
	 public function deleteForgotPassIPAddress($ip=null,$queryString=null)
	 {
		if(isset($queryString) && $queryString!="")
		{
			$Query = "DELETE FROM `forgotpass_ipaddress_attempts` WHERE lastlogin <= DATE_SUB(NOW(), ".FORGOTPASS_IP_TIME_PERIOD.")";
		}else{
			$Query = "delete from forgotpass_ipaddress_attempts where ip = '$ip'";
		}
		//echo $Query;
		//exit();		
		$ThrowException = null;
	 ...
	 * 
	 */
	public function getForgotPassIPAddress($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $QueryString=null, $ThrowError=true)
	{
		/**
		 * Table Columns
		...
		if($QueryString!=''){
			...
			$Query = "SELECT $fields FROM forgotpass_ipaddress_attempts $whereExtra  $optstring $limitString";
		}
		//echo $Query;
		//exit;
		/**
		...
		try	{
			...
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Get Any Record of forgotpass_ipaddress_attempts');
			}
	    } // try -catch end
	    ...
	    if($QueryString!=''){
    		...
    		//assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$ForgotPassIPAddressData[]=$Record;
		    }//foreach end
	    }else{
		    //Create CategoryData Object
	   	    $ForgotPassIPAddressData = new RModelCollection();
	   	    
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$ForgotPassIPAddressData->Add(new ForgotPassIPAddressData($Record));
		    }//foreach end
	    }
	    
	    return $ForgotPassIPAddressData;//return the fetched recordset		
