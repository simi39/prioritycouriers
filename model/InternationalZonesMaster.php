<?php
/**
 * This is InternationalZonesMaster file.
 * This file contains all the business logic code related to international_zones table.
 * 
 * TABLE_NAME:    international_zones 
 * PRIMARY_KEY :  id
 * TABLE_COLUMNS: "id","country","zone","days"
 * 
 * @uses By creating object of InternationalZonesMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    InternationalZones Management
 * @author     Radixweb
 * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb
 * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License
 * @version    $Id: InternationalZonesMaster.php,v 1.0
...
/**
 * @see InternationalZonesData.php for Data class
 */
require_once(DIR_WS_MODEL."InternationalZonesData.php");
...
class InternationalZonesMaster extends RMasterModel
{	
	
	/**
	 * Create a null object of InternationalZonesMaster
	 *
	 * @return object object $InternationalZonesMasterObj
	 */	
	public function create()
	{
		$InternationalZonesMasterObj = new InternationalZonesMaster();
		return  $InternationalZonesMasterObj;
	}//create Function End
	      
	/**
	 * To insert record in international_zones table 
	 * Insert data of columns : "id","country","zone","days"
	 *
	 * @param arrayInternationalZonesData $InternationalZonesData 
	 ...
	 */	
	 public function addInternationalZones($InternationalZonesData, $ThrowError=true)
	 {
	 		//build up insert query
		 	$FinalData = $InternationalZonesData->InternalSync(RDataModel::INSERT, "id","country","zone","days");
		 	$Query = "INSERT INTO international_zones $FinalData"; 
	 		...
	 		try { 
				...
				if ($Rs->AffectedRows() ==  0) {
					throw new Exception(' Fail To Add New international_zones Record');
				}
				...
				if($ThrowError == true){
					$ThrowException = new Exception(' Fail To Add international_zones Record ');
				} else {
					return false;	
				}//if-else end
	...
	}//addInternationalZones Function End
	
	
	
	/**
	 * To update record of international_zones table
	 * Update data of columns : "id","country","zone","days"
	 * 
	 * @param arrayInternationalZonesData $InternationalZonesData 
	 * @param bool $ThrowError Whether to throw error or not
	 * 
	 ...
	 */	
	public function editInternationalZones($InternationalZonesData, $ThrowError=true) 
	{
		 
		//build up update query
		$UpdateData = $InternationalZonesData->InternalSync(RDataModel::UPDATE, "id","country","zone","days");         
        $Query = "UPDATE international_zones SET $UpdateData WHERE id = ". $InternationalZonesData->id; 
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit international_zones Record ');
				return false;
	  		}
		}//try end
		catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit international_zones Record ');
			 } else {
				return false;	
			 }
    	}// try-catch end
	...
	}//editInternationalZones Function End
	
	
	/**
	 * To delete international_zones data
	 *
	 * @param int id $id 
	 ...
	 * @return bool If any error, then Throw Exception otherwise return true on success and false on failure
	 */	 
	public function deleteInternationalZones($id, $ThrowError=true) 
	{
   		if(isset($id) && ($id!=null)) {
 			
   			//build up delete query
   			$Query ="DELETE FROM international_zones WHERE id = " . $id;
 			...
 			try {	
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete international_zones Record');
				}	
			    else {
					return false;	
			    }
	 ...
	 }//deleteInternationalZones Function End	
	
	 
	 /**
	 * GetInternationalZones method is used for to get all or selected columns records from international_zones with searching, sorting and paging criteria.
	 *
	 ...
	 * 
	 */
	 public function getInternationalZones($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true,$uniqe_data=false)
	 {
			 ...
			 * Build up query
			 */
 $Query = "SELECT $fields FROM international_zones $whereExtra $optstring $limitString";
			...
			try	{
				...
				if($ThrowError == true) {
					$ThrowException = new Exception(' Fail To Get Any Record of international_zones');
				}
		    } // try -catch end
	  
		    //Create CategoryData Object
	   	    $InternationalZonesData = new RModelCollection();
	   	    
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$InternationalZonesData->Add(new InternationalZonesData($Record));
		    }//foreach end
		    
		    return $InternationalZonesData;//return the fetched recordset		
	 }//getInternationalZones function end 
