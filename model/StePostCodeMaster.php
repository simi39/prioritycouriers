<?php
/**
  * This is StePostCodeMaster file.  
  * This file contains all the business logic code related to postcode_ste table.  
  *   
  * TABLE_NAME:    postcode_ste   
  * PRIMARY_KEY :  auto_id  
  * TABLE_COLUMNS: "Postcode","FullName","Zone"  
  *   
  * @uses By creating object of StePostCodeMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.  
  *   
  * @package    Specify package to group classes or functions and defines into  
  * @author     AUTHOR_NAME  
  * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb  
  * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License  
  * @version    $Id: StePostCodeMaster.php,v 1.0  
  * @link       Display a hyperlink to a URL in the documentation, e.g. http://radixweb.com  
  * @since      Document when (at which version) an element was first added to a package e.g. Version 1.0   
  *   
  */   
  /**  
  * @see StePostCodeData.php for Data class  */ 
  ...
  * @see RMasterModel.php for extended RMasterModel class  */
  ...
  class StePostCodeMaster extends RMasterModel 
  {     
  /**   
  * Create a null object of StePostCodeMaster   
  *  
  * @return object object $StePostCodeMasterObj   */  
   public function create()  
   {   
  		$StePostCodeMasterObj = new StePostCodeMaster();   
		return  $StePostCodeMasterObj;  
   }
   //create Function End          
   /**  
   * To insert record in postcode_ste table   
   * Insert data of columns : "Postcode","FullName","Zone"   
   *   
   * @param arrayStePostCodeData $StePostCodeData   
   ...
   */
	...
	public function addStePostCode($StePostCodeData, $ThrowError=true)   
	{    
		 //build up insert query     
		 $FinalData = $StePostCodeData->InternalSync(RDataModel::INSERT, "Postcode","FullName","DirectZone","Zone");     
		  $Query = "INSERT INTO postcode_ste $FinalData";
		 /**
		 ...
		 */     
		 ...
		 {            
		 		...
		 		{      
		 			throw new Exception(' Fail To Add New postcode_ste Record');    
		 		}         
		 }    
		 ...
		 {          
			if($ThrowError == true)
			{      
				$ThrowException = new Exception(' Fail To Add postcode_ste Record ');     
			} 
			else 
			{      
				return false;      
	...
	}//addStePostCode Function End        
	/**   
	* To update record of postcode_ste table   
	* Update data of columns : "Postcode","FullName","Zone"   
	*    
	* @param arrayStePostCodeData $StePostCodeData    
	* @param bool $ThrowError Whether to throw error or not   
	*    
	* @return bool If any error, then Throw Exception otherwise return true on success and false on failure   */
	...
	public function editStePostCode($StePostCodeData, $ThrowError=true)   
	{       
	//build up update query   
		$UpdateData = $StePostCodeData->InternalSync(RDataModel::UPDATE, "Postcode","FullName","DirectZone","Zone");
		$Query = "UPDATE postcode_ste SET $UpdateData WHERE auto_id = ". $StePostCodeData->auto_id;
		...
		* if no then throw or return proper error message.     */
		...
		try 
		{
		      ...
		      if ( $Rs->AffectedRows() == 0 ) 
		      {     
		      		//throw new Exception(' Fail To Edit postcode_ste Record ');
		      		return false;
		      }  
		}//try end   
		catch(Exception $Exception)
		{
		    if($ThrowError == true) 
		    {
		         $ThrowException = new Exception(' Fail To Edit postcode_ste Record ');
			} else 
			{     return false;      }      
		}// try-catch end
		if ($ThrowException)
		...
		{		    return true;   }     
	}//editStePostCode Function End      
	
/**
* To delete postcode_ste data   
*   
...
* @return bool If any error, then Throw Exception otherwise return true on success and false on failure   
*/    
public function deleteStePostCode($auto_id, $ThrowError=true)
{
      if(isset($auto_id) && ($auto_id!=null))
      {            //build up delete query
			$Query ="DELETE FROM postcode_ste WHERE auto_id = " . $auto_id;
			/**
			* check that data should be updated perfectly.      
			* if no then throw or return proper error message.       */     
			$ThrowException = null;     
			...
			catch (Exception $Exception)
			{
			     if($ThrowError == true) 
			     {
			              $ThrowException = new Exception(' Fail To Delete postcode_ste Record');     
			     }         
			     else 
			     {      return false;         }    
			}// try catch end               
...
}//deleteStePostCode Function End         


/**   
* GetStePostCode method is used for to get all or selected columns records from postcode_ste with searching, sorting and paging criteria.   
*   
...
public function getStePostCode($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true)   
{       
	/**     
	* Table Columns     
...
* Build up query     
*/    
  $Query = "SELECT $fields FROM postcode_ste $whereExtra $optstring $limitString";
  //echo $Query;
/**
* Execute Query     
...
catch (Exception $Exception)
{
     if($ThrowError == true) 
     {
           $ThrowException = new Exception(' Fail To Get Any Record of postcode_ste');
     }       
} // try -catch end
 //Create CategoryData Object          
 
$StePostCodeData = new RModelCollection();
	//assing all fetched records into CategoryData Object          
	if($Rs!="")
	{
	foreach ($Rs as $Record) 
	{
	      $StePostCodeData->Add(new StePostCodeData($Record));       
	}//foreach end
	}
	return $StePostCodeData;//return the fetched recordset     
}
//getStePostCode function end          
