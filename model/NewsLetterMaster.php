<?php
/**
  * This is NewsLetterMaster file.  
  * This file contains all the business logic code related to newsletter table.  
  *   
  * TABLE_NAME:    newsletter   
  * PRIMARY_KEY :  id  
  * TABLE_COLUMNS: "id","email_id","status" 
  *   
  * @uses By creating object of NewsLetterMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.  
  *   
  * @package    Specify package to group classes or functions and defines into  
  * @author     AUTHOR_NAME  
  * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb  
  * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License  
  * @version    $Id: NewsLetterMaster.php,v 1.0  
  * @link       Display a hyperlink to a URL in the documentation, e.g. http://radixweb.com  
  * @since      Document when (at which version) an element was first added to a package e.g. Version 1.0   
  *   
  */   
  /**  
  * @see NewsLetterData.php for Data class  */ 
  require_once(DIR_WS_MODEL."/NewsLetterData.php");  
  /**  
  * @see RMasterModel.php for extended RMasterModel class  */

  require_once(DIR_LIB_CLASSES_MODELS."/RMasterModel.php"); 

  class NewsLetterMaster extends RMasterModel 
  {     
  /**   
  * Create a null object of NewsLetterMaster   
  *  
  * @return object object $NewsLetterMasterObj   */  
   public function create()  
   {   
  		$NewsLetterMasterObj = new NewsLetterMaster();   
		return  $NewsLetterMasterObj;  
   }
   //create Function End          
   /**  
   * To insert record in newsletter table   
   * Insert data of columns : "id","email_id","status"   
   *   
   * @param arrayNewsLetterData $NewsLetterData   
   ...
   */
	...
	public function addNewsLetter($NewsLetterData, $ThrowError=true)   
	{    
		 //build up insert query     
		 $FinalData = $NewsLetterData->InternalSync(RDataModel::INSERT, "id","email_id","status");     
		 $Query = "INSERT INTO newsletter $FinalData";//echo $Query;           
		 /**
		 ...
		 */     
		 ...
		 {            
		 		...
		 		{      
		 			throw new Exception(' Fail To Add New newsletter Record');    
		 		}         
		 }    
		 ...
		 {          
			if($ThrowError == true)
			{      
				$ThrowException = new Exception(' Fail To Add newsletter Record ');     
			} 
			else 
			{      
				return false;      
	...
	}//addNewsLetter Function End        
	/**   
	* To update record of newsletter table   
	* Update data of columns : "id","email_id","status"   
	*    
	* @param arrayNewsLetterData $NewsLetterData    
	* @param bool $ThrowError Whether to throw error or not   
	*    
	* @return bool If any error, then Throw Exception otherwise return true on success and false on failure   */
	...
	public function editNewsLetter($NewsLetterData, $ThrowError=true)   
	{       
	//build up update query  
	
	if($statusChange != ""){
			$UpdateData = $NewsLetterData->InternalSync(RDataModel::UPDATE,"status");  
		} else {
			$UpdateData = $NewsLetterData->InternalSync(RDataModel::UPDATE,"id");         
	    }		
		//$UpdateData = $NewsData->InternalSync(RDataModel::UPDATE, "newscat_id","status","sort_order");
		$Query = "UPDATE newsletter SET $UpdateData WHERE id = ". $NewsLetterData->id;
		...
		* if no then throw or return proper error message.     */
		...
		catch(Exception $Exception)
		{
		    if($ThrowError == true) 
		    {
		         $ThrowException = new Exception(' Fail To Edit newsletter Record ');
			} else 
			{     return false;      }      
		}// try-catch end
		if ($ThrowException)
...
* @return bool If any error, then Throw Exception otherwise return true on success and false on failure   
*/    
public function deleteNewsLetter($id, $ThrowError=true)
{
      if(isset($id) && ($id!=null))
      {            //build up delete query
			$Query ="DELETE FROM newsletter WHERE id = " . $id;
   			$DesQuery ="DELETE FROM newsletter WHERE id = " . $id;
			/**
			* check that data should be updated perfectly.      
			* if no then throw or return proper error message.       */     
			$ThrowException = null;     
			...
			catch (Exception $Exception)
			{
			     if($ThrowError == true) 
			     {
			              $ThrowException = new Exception(' Fail To Delete newsletter Record');     
			     }         
			     else 
			     {      return false;         }    
			}// try catch end               
...
}//deleteNewsLetter Function End         


/**   
* GetNewsLetter method is used for to get all or selected columns records from newsletter with searching, sorting and paging criteria.   
*   
...
public function getNewsLetter($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true,$table_name='newsletter')   
{       
	/**     
	* Table Columns     
...
catch (Exception $Exception)
{
     if($ThrowError == true) 
     {
           $ThrowException = new Exception(' Fail To Get Any Record of newsletter');
     }       
} // try -catch end
 //Create CategoryData Object          
 
$NewsLetterData = new RModelCollection();
	//assing all fetched records into CategoryData Object          
	
	foreach ($Rs as $Record) 
	{
	      $NewsLetterData->Add(new NewsLetterData($Record));       
	}//foreach end
	return $NewsLetterData;//return the fetched recordset     
}
