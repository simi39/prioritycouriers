<?php
/**
 * This is CcDetailsMaster file.
 * This file contains all the business logic code related to ccdetails table.
 * 
 * TABLE_NAME:    ccdetails 
 * PRIMARY_KEY :  cc_id
 * TABLE_COLUMNS: cc_id,userid,hname,ccnumber,expdate
 * 
 * @uses By creating object of CcDetailsMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    CcDetails
 * @author     Radixweb
 * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb
 * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License
 * @version    $Id: CcDetailsMaster.php,v 1.0
...
/**
 * @see CcDetailsData.php for Data class
 */
require_once(DIR_WS_MODEL."/CcDetailsData.php");
...
class CcDetailsMaster extends RMasterModel
{	
	
	/**
	 * Create a null object of CcDetailsMaster
	 *
	 * @return object object $CcDetailsMasterObj
	 */	
	public function create()
	{
		$CcDetailsMasterObj = new CcDetailsMaster();
		return  $CcDetailsMasterObj;
	}//create Function End
	      
	/**
	 * To insert record in ccdetails table 
	 * Insert data of columns : cc_id,userid,hname,ccnumber,expdate
	 *
	 * @param arrayCcDetailsData $CcDetailsData 
	 ...
	 */	
	 public function addCcDetails($CcDetailsData, $ThrowError=true)
	 {
	 		//build up insert query
		 	$FinalData = $CcDetailsData->InternalSync(RDataModel::INSERT, cc_id,userid,hname,ccnumber,expdate,cardtype);
		 	$Query = "INSERT INTO ccdetails $FinalData"; 
	 		...
	 		try { 
				...
				if ($Rs->AffectedRows() ==  0) {
					throw new Exception(' Fail To Add New ccdetails Record');
				}
				...
				if($ThrowError == true){
					$ThrowException = new Exception(' Fail To Add ccdetails Record ');
				} else {
					return false;	
				}//if-else end
	...
	}//addCcDetails Function End
	
	
	
	/**
	 * To update record of ccdetails table
	 * Update data of columns : cc_id,userid,hname,ccnumber,expdate
	 * 
	 * @param arrayCcDetailsData $CcDetailsData 
	 * @param bool $ThrowError Whether to throw error or not
	 * 
	 ...
	 */	
	public function editCcDetails($CcDetailsData, $ThrowError=true) 
	{
		 
		//build up update query
		$UpdateData = $CcDetailsData->InternalSync(RDataModel::UPDATE, cc_id,userid,hname,ccnumber,expdate,cardtype);         
       	$Query = "UPDATE ccdetails SET $UpdateData WHERE cc_id = ". $CcDetailsData->cc_id;      
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit ccdetails Record ');
				return false;
	  		}
		}//try end
		catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit ccdetails Record ');
			 } else {
				return false;	
			 }
    	}// try-catch end
	...
	}//editCcDetails Function End
	
	
	/**
	 * To delete ccdetails data
	 *
	 * @param int cc_id $cc_id 
	 * @param bool $ThrowError Whether to throw or not to throw error
	 * 
	 * @return bool If any error, then Throw Exception otherwise return true on success and false on failure
	 */	 
	public function deleteCcDetails($cc_id, $ThrowError=true) 
	{
   		if(isset($cc_id) && ($cc_id!=null)) {
 			
   			//build up delete query
   			$Query ="DELETE FROM ccdetails WHERE cc_id = " . $cc_id;
 			...
 			try {	
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete ccdetails Record');
				}	
			    else {
					return false;	
			    }
	 ...
	 }//deleteCcDetails Function End	
	
	 
	 /**
	 * GetCcDetails method is used for to get all or selected columns records from ccdetails with searching, sorting and paging criteria.
	 *
	 ...
	 * 
	 */
	 public function getCcDetails($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true)
	 {
			 ...
			 * Build up query
			 */
			$Query = "SELECT $fields FROM ccdetails $whereExtra $optstring $limitString";
			...
			try	{
				...
				if($ThrowError == true) {
					$ThrowException = new Exception(' Fail To Get Any Record of ccdetails');
				}
		    } // try -catch end
	  
		    //Create CategoryData Object
	   	    $CcDetailsData = new RModelCollection();
	   	    
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$CcDetailsData->Add(new CcDetailsData($Record));
		    }//foreach end
		    
		    return $CcDetailsData;//return the fetched recordset		
	 }//getCcDetails function end 
