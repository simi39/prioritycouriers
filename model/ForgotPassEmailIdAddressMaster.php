<?php
/**
 * This is ForgotPassEmailIdAddressMaster file.
 * This file contains all the business logic code related to  forgotpass_email_attempts table.
 * 
 * TABLE_NAME:     forgotpass_email_attempts
 * PRIMARY_KEY :  userid
 * TABLE_COLUMNS: 
 * 
 * @uses By creating object of ForgotPassEmailIdAddressMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    Forgot Password Email Attempts Management
...
/**
 * @see ForgotPassEmailIdData.php for Data class
 */
require_once(DIR_WS_MODEL . "ForgotPassEmailIdAddressData.php");
...
class ForgotPassEmailIdAddressMaster extends RMasterModel
{
	/**
	 * Create a null object of ForgotPassEmailIdAddressMaster
	 *
	 * @return object object $ForgotPassEmailIdAddressMasterObj
	 */
	public function create()
	{
		$ForgotPassEmailIdAddressMasterObj = new ForgotPassEmailIdAddressMaster();
		return  $ForgotPassEmailIdAddressMasterObj;
	}//create Function End

	/**
	 * To insert record in forgotpass_ipaddress_attempts table
	 * 
	 *
	 ...
	 */
	public function addForgotPassEmailIdAddress($ForgotPassIPAddressData, $ThrowError=true,$QueryString=null)
	{
		if(empty($QueryString))
		{
			//build up insert query
			$FinalData = $ForgotPassEmailIdAddressData->InternalSync(RDataModel::INSERT,"emailid","attempts","lastlogin");
			$Query = "INSERT INTO forgotpass_email_attempts $FinalData";
		}
		else
		{
			$Query = $QueryString;	
 		...
 		try {
			...
			if ($Rs->AffectedRows() ==  0) {
				throw new Exception(' Fail To Add New forgotpass_email_attempts
 Record');
			}
		} catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Add forgotpass_email_attempts Record ');
			} else {
				return false;
			}//if-else end
		}// try -catch end
	 ...
	 */
	public function editForgotPassEmailIdAddress($ForgotPassEmailIdAddressData=null, $QueryString=null, $ThrowError=true)
	{
		...
		if($QueryString)
		{
			 ...
			 $UpdateData = $ForgotPassEmailIdAddressData->InternalSync(RDataModel::UPDATE, "attempts","lastlogin");
			 $Query = 'UPDATE forgotpass_email_attempts
 SET '.$UpdateData.' WHERE emailid = "'.$ForgotPassEmailIdAddressData->emailid.'"';
		}
		//echo $Query;
		//exit();
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit forgotpass_email_attempts Record ');
				return false;
	  		}
		} catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit forgotpass_email_attempts Record ');
			 } else {
				return false;
			 }
    	}// try-catch end
	 ...
	 *
	 */
	 public function deleteForgotPassEmailIdAddress($emailid=null,$queryString=null)
	 {
		
		if(isset($queryString) && $queryString!="")
		{
			$Query = "DELETE FROM `forgotpass_email_attempts` WHERE lastlogin <= DATE_SUB(NOW(), ".FORGOTPASS_TIME_PERIOD.")";
			
		}else{
			$Query ="delete from forgotpass_email_attempts where emailid = '$emailid'";
		}
		//echo $Query;
		//exit();
	 ...
	 * 
	 */
	public function getForgotPassEmailIdAddress($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $QueryString=null, $ThrowError=true)
	{
		/**
		 * Table Columns
		...
		if($QueryString!=''){
			...
			$Query = "SELECT $fields FROM forgotpass_email_attempts $whereExtra  $optstring $limitString";
		}
		//echo $Query;
		//exit;
		/**
		...
		try	{
			...
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Get Any Record of forgotpass_email_attempts');
			}
	    } // try -catch end
	    ...
	    if($QueryString!=''){
    		...
    		//assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$ForgotPassEmailIdAddressData[]=$Record;
		    }//foreach end
	    }else{
		    //Create CategoryData Object
	   	    $ForgotPassEmailIdAddressData = new RModelCollection();
	   	    
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$ForgotPassEmailIdAddressData->Add(new ForgotPassEmailIdAddressData($Record));
		    }//foreach end
	    }
	    
	    return $ForgotPassEmailIdAddressData;//return the fetched recordset		
