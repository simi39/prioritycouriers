...
/**
 * This is ServiceCutoffCutoffMaster file.
 * This file contains all the business logic code related to service_cutoff  table.
 * 
 * TABLE_NAME:    service_cutoff 
 * PRIMARY_KEY :  auto_id
 * TABLE_COLUMNS: "service_cutoff_name","hours","minites","hr_formate"    
 *  
 * 
 * @uses By creating object of ServiceCutoffMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    ServiceCutoff Managment
 * @author     Radix
 * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb
 * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License
 * @version    $Id: ServiceCutoffMaster.php,v 1.0
...
/**
 * @see ServiceCutoffData.php for Data class
 */
require_once(DIR_WS_MODEL."/ServiceCutoffData.php");
...
class ServiceCutoffMaster extends RMasterModel
{	
	
	/**
	 * Create a null object of ServiceCutoffMaster
	 *
	 * @return object object $ServiceCutoffMasterObj
	 */	
	public function create()
	{
		$ServiceCutoffMasterObj = new ServiceCutoffMaster();
		return  $ServiceCutoffMasterObj;
	}//create Function End
	      
	/**
	 * To insert record in service_cutoff table 
	 * Insert data of columns : "service_cutoff_name","hours","minites","hr_formate"
	 *
	 * @param arrayServiceCutoffData $ServiceCutoffData 
	 ...
	 */	
	 public function addServiceCutoff($ServiceCutoffData, $ThrowError=true)
	 {
	 		//build up insert query
 		 	$FinalData = $ServiceCutoffData->InternalSync(RDataModel::INSERT, "service_cutoff_name","hours","minites","hr_formate");
		 	$Query = "INSERT INTO service_cutoff $FinalData"; 		 
		 	/**
	 		...
	 		try { 
				...
				if ($Rs->AffectedRows() ==  0) {
					throw new Exception(' Fail To Add New service_cutoff Record');
				}
				...
				if($ThrowError == true){
					$ThrowException = new Exception(' Fail To Add service_cutoff Record ');
				} else {
					return false;	
				}//if-else end
	...
	}//addServiceCutoff Function End
	
	
	
	/**
	 * To update record of service_cutoff table
	 * Update data of columns : "service_cutoff_name","hours","minites","hr_formate"
	 * 
	 * @param arrayServiceCutoffData $ServiceCutoffData 
	 * @param bool $ThrowError Whether to throw error or not
	 * 
	 ...
	 */	
	public function editServiceCutoff($ServiceCutoffData, $statusChange=null, $ThrowError=true) 
	{
		 
		//build up update query
		if($statusChange == true) {
 			$UpdateData = $ServiceCutoffData->InternalSync(RDataModel::UPDATE, "status");
 		} else {
			$UpdateData = $ServiceCutoffData->InternalSync(RDataModel::UPDATE, "hours","minites","hr_formate");
	        	$Query = "UPDATE service_cutoff SET $UpdateData WHERE auto_id = ". $ServiceCutoffData->auto_id;
 		}
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit service_cutoff Record ');
				return false;
	  		}
		}//try end
		catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit service_cutoff Record ');
			 } else {
				return false;	
			 }
    	}// try-catch end
	...
	}//editServiceCutoff Function End
	
	
	
	/**
	 * To delete service_cutoff data
	 *
	 ...
	 * @return bool If any error, then Throw Exception otherwise return true on success and false on failure
	 */	 
	public function deleteServiceCutoff($auto_id, $ThrowError=true) 
	{
   		if(isset($auto_id) && ($auto_id!=null)) {
 			
   			//build up delete query
   			$Query ="DELETE FROM service_cutoff WHERE auto_id = " . $auto_id;
 			...
 			try {	
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete service_cutoff Record');
				}	
			    else {
					return false;	
			    }
	 ...
	 }//deleteServiceCutoff Function End	
	
	 
	 	 
	 /**
	 * GetServiceCutoff method is used for to get all or selected columns records from service_cutoff with searching, sorting and paging criteria.
	 *
	 ...
	 * 
	 */
	 public function getServiceCutoff($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $QueryString=null , $TableName=null, $ThrowError=true)
	 {
			...
			if(!empty($QueryString)) { 
				...
				if($TableName=="")
				{
					$Query = "SELECT $fields FROM service_cutoff $whereExtra $optstring $limitString";		
				}
				else {
			...
			try	{
				...
				if($ThrowError == true) {
					$ThrowException = new Exception(' Fail To Get Any Record of service_cutoff');
				}
		    } // try -catch end
	  
		    //Create CategoryData Object
	   	    $ServiceCutoffData = new RModelCollection();
	   	    if($Rs!="")
	   	    {
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$ServiceCutoffData->Add(new ServiceCutoffData($Record));
		    }//foreach end
	   	    }
		    
		    return $ServiceCutoffData;//return the fetched recordset		
	 }//getServiceCutoff function end 
