<?php
/**
 * This is PostCodeMaster file.
 * This file contains all the business logic code related to postcode_locator table.
 * 
 * TABLE_NAME:    postcode_locator 
 * PRIMARY_KEY :  TABLE_PRIMARY_KEY
 * TABLE_COLUMNS: "Id","FullName","Name","PostCode","Longitude","Latitude","Group","State","IsInShortList","CopiedFromSuburbId","IsUserLocation","IsEnabled","AddedByUserId"
 * 
...
/**
 * @see PostCodeData.php for Data class
 */
require_once(DIR_WS_MODEL . "PostCodeData.php");
...
class PostCodeMaster extends RMasterModel
{	
	
	/**
	 * Create a null object of PostCodeMaster
	 *
	 * @return object object $PostCodeMasterObj
	 */	
	public function create()
	{
		$PostCodeMasterObj = new PostCodeMaster();
		return  $PostCodeMasterObj;
	}//create Function End
	      
	/**
	 * To insert record in postcode_locator table 
	 * Insert data of columns : "Id","FullName","Name","PostCode","Longitude","Latitude","Group","State","IsInShortList","CopiedFromSuburbId","IsUserLocation","IsEnabled","AddedByUserId"
	 *
	 * @param arrayPostCodeData $PostCodeData 
	 ...
	 */	
	 public function addPostCode($PostCodeData, $ThrowError=true)
	 {
	 		//build up insert query
		 	$FinalData = $PostCodeData->InternalSync(RDataModel::INSERT, "Id","FullName","Name","Postcode","State","Zone","charging_zone","time_zone");
		 	$Query = "INSERT INTO postcode_locator $FinalData"; 
	...
	/**
	 * To update record of postcode_locator table
	 * Update data of columns : "Id","FullName","Name","PostCode","Longitude","Latitude","Group","State","IsInShortList","CopiedFromSuburbId","IsUserLocation","IsEnabled","AddedByUserId"
	 * 
	 * @param arrayPostCodeData $PostCodeData 
	 * @param bool $ThrowError Whether to throw error or not
	 * 
	 ...
	 */	
	public function editPostCode($PostCodeData, $ThrowError=true) 
	{
		 
       
		//build up update query
		
		$UpdateData = $PostCodeData->InternalSync(RDataModel::UPDATE,"Id","Name","Postcode","State","Zone","charging_zone","time_zone","FullName");         
     	$Query = "UPDATE postcode_locator SET $UpdateData WHERE auto_id = ". $PostCodeData->auto_id;      
       	/**
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit postcode_locator Record ');
				return false;
	  		}
		}//try end
		catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit postcode_locator Record ');
			 } else {
				return false;	
			 }
    	}// try-catch end
	...
	/**
	 * To delete postcode_locator data
	 *
	 ...
	 * @return bool If any error, then Throw Exception otherwise return true on success and false on failure
	 */	 
	public function deletePostCode($TABLE_PRIMARY_KEY, $ThrowError=true) 
	{
   		if(isset($TABLE_PRIMARY_KEY) && ($TABLE_PRIMARY_KEY!=null)) {
 			
   			//build up delete query
   			 $Query ="DELETE FROM postcode_locator WHERE auto_id = " . $TABLE_PRIMARY_KEY;
 			...
 			try {	
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete postcode_locator Record');
				}	
			    else {
					return false;	
			    }
	 ...
	 * 
	 */
	 public function getPostCode($fieldArr=null,$tot=false, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true,$uniqe_data=false,$display_all_from_admin=false)
	 {
			...
			$Query = "SELECT $fields FROM postcode_locator $whereExtra $optstring $limitString"; 
			//echo $Query;
			//exit();
		    ...
		    //Create CategoryData Object
	   	    $PostCodeData = new RModelCollection();
	   	    if($Rs!="")
	   	    {
		   	    //assing all fetched records into CategoryData Object
		   	    foreach ($Rs as $Record) {
				 	$PostCodeData->Add(new PostCodeData($Record));
			    }//foreach end
	   	    }    
		    
		  
		    	return $PostCodeData;//return the fetched recordset		
