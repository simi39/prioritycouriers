<?php
/**
 * This is TransitGuideRoadExpressMaster file.
 * This file contains all the business logic code related to transitguide_roadexpress table.
 * 
 * transitguide_roadexpress:    transitguide_roadexpress 
 * PRIMARY_KEY :  auto_id
 * TABLE_COLUMNS: "State","Zone","days"
 * 
 * @uses By creating object of TransitGuideRoadExpressMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    Specify package to group classes or functions and defines into
 * @author     AUTHOR_NAME
 * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb
 * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License
 * @version    $Id: TransitGuideRoadExpressMaster.php,v 1.0
...
/**
 * @see TransitGuideRoadExpressData.php for Data class
 */
require_once(DIR_WS_MODEL."/TransitGuideRoadexpressData.php");
...
class TransitGuideRoadExpressMaster extends RMasterModel
{	
	
	/**
	 * Create a null object of TransitGuideRoadExpressMaster
	 *
	 * @return object object $TransitGuideRoadExpressMasterObj
	 */	
	public function create()
	{
		$TransitGuideRoadExpressMasterObj = new TransitGuideRoadExpressMaster();
		return  $TransitGuideRoadExpressMasterObj;
	}//create Function End
	      
	/**
	 * To insert record in transitguide_roadexpress table 
	 * Insert data of columns : "State","Zone","days"
	 *
	 * @param arrayTransitGuideRoadExpressData $TransitGuideRoadExpressData 
	 ...
	 */	
	 public function addTransitGuideOvernight($TransitGuideRoadExpressData, $ThrowError=true)
	 {
	 		//build up insert query
		 	$FinalData = $TransitGuideRoadExpressData->InternalSync(RDataModel::INSERT, "State","Zone","days");
		 	$Query = "INSERT INTO transitguide_roadexpress $FinalData"; 
	 		...
	 		try { 
				...
				if ($Rs->AffectedRows() ==  0) {
					throw new Exception(' Fail To Add New transitguide_roadexpress Record');
				}
				...
				if($ThrowError == true){
					$ThrowException = new Exception(' Fail To Add transitguide_roadexpress Record ');
				} else {
					return false;	
				}//if-else end
	...
	/**
	 * To update record of transitguide_roadexpress table
	 ...
	 */	
	public function editTransitGuideOvernight($TransitGuideRoadExpressData, $ThrowError=true) 
	{
		 
		//build up update query
		$UpdateData = $TransitGuideRoadExpressData->InternalSync(RDataModel::UPDATE, "State","Zone","days");         
       	$Query = "UPDATE transitguide_roadexpress SET $UpdateData WHERE auto_id = ". $TransitGuideRoadExpressData->auto_id;      
		...
		try {
	  		$Rs = $this->Connection->Execute($Query);
	  		if ( $Rs->AffectedRows() == 0 )	{
				//throw new Exception(' Fail To Edit transitguide_roadexpress Record ');
				return false;
	  		}
		}//try end
		catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Edit transitguide_roadexpress Record ');
			 } else {
				return false;	
			 }
    	}// try-catch end
	...
	/**
	 * To delete transitguide_roadexpress data
	 *
	...
	public function deleteTransitGuideOvernight($auto_id, $ThrowError=true) 
	{
   		if(isset($auto_id) && ($auto_id!=null)) {
   			...
   			$Query ="DELETE FROM transitguide_roadexpress WHERE auto_id = " . $auto_id;
 			...
 			try {	
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete transitguide_roadexpress Record');
				}	
			    else {
					return false;	
			    }
	 ...
	 /**
	 * GetTransitGuideOvernight method is used for to get all or selected columns records from transitguide_roadexpress with searching, sorting and paging criteria.
	 *
	 ...
	 */
	 public function getTransitGuideOvernight($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true)
	 {
			 ...
			 * Build up query
			 */
			$Query = "SELECT $fields FROM transitguide_roadexpress $whereExtra $optstring $limitString";
			...
			try	{
				...
				if($ThrowError == true) {
					$ThrowException = new Exception(' Fail To Get Any Record of transitguide_roadexpress');
				}
		    } // try -catch end
	  
		    //Create CategoryData Object
	   	    $TransitGuideRoadExpressData = new RModelCollection();
	   	    
	   	    //assing all fetched records into CategoryData Object
	   	    foreach ($Rs as $Record) {
			 	$TransitGuideRoadExpressData->Add(new TransitGuideRoadExpressData($Record));
		    }//foreach end
		    
		    return $TransitGuideRoadExpressData;//return the fetched recordset		
