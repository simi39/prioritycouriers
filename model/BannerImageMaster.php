<?php
/**
 * This is BannerImageMaster file.
 ...
 * 
 * @uses By creating object of BannerImageMaster we can handle or manage different table operation like insert, update, delete and selecting data with criteria from table.
 * 
 * @package    Banner Image Management
 * @author     Radixweb
 * @copyright  Document Copyright information, e.g. Copyright (c) 2008, Radixweb
 * @license    Display a hyperlink to a URL for a license, e.g. http://opensource.org/licenses/gpl-license.php GNU Public License
 * @version    $Id: BannerImageMaster.php,v 1.0
...
/**
 * @see BannerImageData.php for Data class
 */
require_once(DIR_WS_MODEL."/BannerImageData.php");
...
class BannerImageMaster extends RMasterModel {
	/**
	 * Create a null object of BannerImageMaster
	 *
	 * @return object object $BannerImageMasterObj
	 */
	public function create()
	{
		$BannerImageMasterObj = new BannerImageMaster();
		return  $BannerImageMasterObj;
	}//create Function End

	/**
	 * To insert record in banner_image table
	 * Insert data of columns : "banner_image_name"
	 *
	 * @param arrayBannerImageData $BannerImageData
	 ...
	 */
	public function addBannerImage($BannerImageData, $ThrowError=true)
	{
		//build up insert query
	 	$FinalData = $BannerImageData->InternalSync(RDataModel::INSERT, "banner_image_name","banner_link","open_target");
	 	$Query = "INSERT INTO banner_image $FinalData";
 		...
 		try {
			...
			if ($Rs->AffectedRows() ==  0) {
				throw new Exception(' Fail To Add New banner image Record');
			}
		} catch(Exception $Exception) {
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Add banner image Record ');
			} else {
				return false;
			}//if-else end
		}// try -catch end
	...
	}//addBannerImage Function End

	/**
	 * To update record of banner_image table
	 * Update data of columns : "banner_image_name"
	 * 
	 * @param arrayBannerImageData $BannerImageData
	 * @param bool $ThrowError Whether to throw error or not
	 * 
	 ...
	 */
	public function editBannerImage($BannerImageData, $ThrowError=true)
	{
		//build up update query
		$UpdateData = $BannerImageData->InternalSync(RDataModel::UPDATE, "banner_image_name","banner_link","open_target");
       	$Query = "UPDATE banner_image SET $UpdateData WHERE banner_image_id = ". $BannerImageData->banner_image_id;
	...
	}//editBannerImage Function End

	/**
	 * To delete banner_image data
	 *
	 * @param int banner_image_id $banner_image_id
	 * @param bool $ThrowError Whether to throw or not to throw error
	 * 
	 * @return bool If any error, then Throw Exception otherwise return true on success and false on failure
	 */
	public function deleteBannerImage($banner_image_id, $ThrowError=true)
	{
   		if(isset($banner_image_id) && ($banner_image_id!=null)) {
   			//build up delete query
   			$Query ="DELETE FROM banner_image WHERE banner_image_id = " . $banner_image_id;
 			...
 			try {
				...
				if($ThrowError == true) {
			    	$ThrowException = new Exception(' Fail To Delete banner image Record');
				} else {
					return false;
			    }
			}// try catch end
	 ...
	 * 
	 */
	public function getBannerImage($fieldArr=null, $seaArr=null, $optArr=null, $start=null, $total=null, $ThrowError=true)
	{
		/**
		 * Table Columns
		 ...
		 * Build up query
		 */
		$Query = "SELECT $fields FROM banner_image $whereExtra $optstring $limitString";
		...
		try	{
			...
			if($ThrowError == true) {
				$ThrowException = new Exception(' Fail To Get Any Record of banner image');
			}
	    } // try -catch end
  
	    //Create CategoryData Object
   	    $BannerImageData = new RModelCollection();
   	    
   	    //assing all fetched records into CategoryData Object
   	    foreach ($Rs as $Record) {
		 	$BannerImageData->Add(new BannerImageData($Record));
	    }//foreach end
	    
	    return $BannerImageData;//return the fetched recordset		
	}//getBannerImage function end
}//class End