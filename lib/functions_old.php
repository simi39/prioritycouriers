...
function show_page_link($pagename=null,$SSL=false)
{
	...
	if($pagename != '' ) {
		...
		if(ENABLE_SSL == true || $SSL == true) {
			//$url = "https://" . SITE_URL_WITHOUT_PROTOCOL . $pagename;
...
function user_athuentication($str_email, $str_pwd) {
	...
	$result = checkIPAddress($clientip,$str_email); // check the ip address for ban
	...
	if(empty($error['error_email']))
	{
		if(!empty($str_email))
		{
			...
			$pass_error = $bcrypt->verify($str_pwd, $HashFromDB);
	...
	if($err_flag == false) {
		...
		if(empty($DataUser)) {
		 ...
		 if($flag_sess) {
			...
			$changeStatus = array("last_login_date","login_attempt");
...
function checkIPAddress($ip,$emailid='')
{
	...
	$IPAddressFieldArr   = array('attempts','(CASE when lastlogin is not NULL and DATE_ADD(LastLogin, INTERVAL '.TIME_PERIOD.' MINUTE)>NOW() then 1 else 0 end) as Denied');
...
function checkEmailAddress($email)
{
	...
	$UserMasterObj = new UserMaster();
	$UserMasterObj = $UserMasterObj->Create();
	$userFieldArr   = array('userid','login_attempt','(CASE when last_login_date is not NULL and DATE_ADD(last_login_date, INTERVAL '.TIME_PERIOD.' MINUTE)>NOW() then 1 else 0 end) as Denied');
	$userSeaArr = array();
...
function default_time_zone($state){
	...
	switch ($state)
			...
			case "NT":
			  date_default_timezone_set("Australia/North");
              $time_array['time']=date('h:i a');
              $time_array['date'] = date('jS F Y');
			  break;
			case "SA":
			  date_default_timezone_set("Australia/South");
			  $time_array['time']=date('h:i a');
...
{	
	...
	if($no!=0){
		...
		///echo "basic price:".$basic_price_table."</br>";
...
function cal_val_direct($base_tariff,$kg,$ste_table,$distance)
{
	...
	if($no!=0){
		...
		//echo "minimum:".$minimum_price_table."basic price".$basic_price_table."kilo rate".$kilo_rate_table;
		//exit();
	...
	if(isset($int_item_type))
	{
		switch($int_item_type)
		{
			case "4":
				...
				{
					$query =  "select cost,multi from $ste_table where weight_from='1000' and zone ='$deliverinterzone' and doc_type='4'";
				}
...
function set_tracking_status(){
	...
	foreach($event_tracking_arr as $tracking_status_id =>$tracking_status){
		if(TRACKING_EVENT_TRANSIT == $tracking_status){
			define("TRANSTI_STATUS_VALUE",$tracking_status_id);
		}
		if(TRACKING_EVENT_DELIVERED == $tracking_status){
...
function sessionX(){
	...
	else
	{
		...
		if($userid)
		{
			...
			$logLength = 600; # time in seconds :: 1800 = 30 minutes 1min
		}else{
			$logLength = 1800;
			//$logLength = 86400;
