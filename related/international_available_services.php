 ...
 //echo "service name:".$service_name." select time:".$selectedTime." service cutoff timing:".$dbtiming."</br>";
	...
	if(isset($final_price)){
		...
		if(isset($serviceName) && $serviceName == strtolower($service_name)){
			...
			if($opacity)
			{
				$str .= '<button type="button" class="btn-u" id="show" name="selectDt" onclick="riseService(\''.$service_code.'\');showButton();javascript:return ChooseInternationalCollectionDate(\''.$k.'\',\''.strtolower(valid_output($service_val->service_name)).'\',\''.$courier_id.'\',\''.number_format($final_price,2,'.','').'\');">'.$button.'</button>';
			}else{
