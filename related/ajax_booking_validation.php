 ...
 */
if(isset($_POST['service_item_type']) && ($_POST['service_item_type'] == 'sameday') || ($_POST['service_item_type'] == 'overnight')){
...
echo $row_no."^".$itemstr."@".$qtyStr."@".$wghtStr."@".$lengthStr."@".$widthStr."@".$heightStr;
exit();
}
...
if(isset($dimArr) && !empty($dimArr) && $Svalidation == false)
{
	foreach($dimArr as $k => $v) {
		...
		if(isset($k) && $k!="")
		{
			...
			if($service_item_type == 'international')
			{
				$item_type = $dimArr['itemtype_1'];
			}else{
				$item_type = $dimArr["itemtype_".$dimKey[1]];
			}
			...
			if($dimKey[0] == 'qty')
			{
				...
				if(!empty($data))
				{
					...
					{
						//echo "qty_".$dimKey[1];
						$qty = $dimArr["qty_".$dimKey[1]];
						$weight = $dimArr["weight_".$dimKey[1]]; 
						$length = $dimArr["length_".$dimKey[1]]; 
						$width = $dimArr["width_".$dimKey[1]]; 
						$height = $dimArr["height_".$dimKey[1]]; 
						
						if(isset($qty) && $servicePageDetail['qty_min']!="" && $servicePageDetail['qty_max']!="")
						{
							if($servicePageDetail['qty_status']=='1' && (trim($qty)<$servicePageDetail['qty_min'] || trim($qty)>$servicePageDetail['qty_max']))
							{
								$qty_min = $servicePageDetail['qty_min'];
								$qty_max = $servicePageDetail['qty_max'];
								$qty_err = 'Quantity limit:<br />up to '.$qty_max.' items';
							}else{
								$qty_err = 'active';
							}
						}
